...
public class PlayerPerks : MonoBehaviour{
    ...
    void MakeSplit(int id){
        //for(var i=0;i<3;i++){
            ...
            go.GetComponent<SplitBullet>().coords=GameSession.instance.players.Where(x => x.playerScript.GetComponent<PlayerScript>().playerNum != GetComponent<PlayerScript>().playerNum).SingleOrDefault().playerScript.transform.position;
            go.GetComponent<SpriteRenderer>().sprite=GetComponent<SpriteRenderer>().sprite;
